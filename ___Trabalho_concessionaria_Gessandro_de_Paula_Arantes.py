...
def vender_veiculo():# IRÁ PARA O MENU
    ...
    if modelo not in preco_carro:
       print("Este veículo não foi encontrado! Venda não realizada.")
       return
    ...
    if escolha == "sim":
       ...
       print(F"Venda do veículo {modelo, marca} realizada com sucesso!")
    else:
...
def comprar_veiculo():
    ...
    if escolha < 0 or escolha >= len(carros_venda):
       ...
       return
    marca = preco_carro[escolha][0]
    modelo = preco_carro[escolha]
    valor_fipe = preco_carro.get(marca, modelo, 0)
